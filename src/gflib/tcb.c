 ...
 */
//--------------------------------------------------------------
BOOL DebugTCBSys_FuncEntryCheck(TCBSYS* tcbsys, TCB_FUNC func)
{
	TCB_PTR now;
	
	now = tcbsys->tcb_first.next;	//ここが実行の始めの場所
	while(now != &(tcbsys->tcb_first)){
		if(now->func == func){
			return TRUE;	//登録済み
		}
		now = tcbsys->next_chain;
	}
	return FALSE;
}
#endif	//PM_DEBUG

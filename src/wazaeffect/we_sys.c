...
enum {
	...
	WE_HDL_ARC_BATT_BG = 0,
	WE_HDL_ARC_BATT_OBJ,
	WE_HDL_ARC_WER_CHAR,
	WE_HDL_ARC_WER_PLTT,
	WE_HDL_ARC_WER_CELL,
	WE_HDL_ARC_WER_CELLANM,
...
WE_SYS_PTR WES_Create(int heap_area)
{
	...
	we_sys->hdl[ 3 ] = ArchiveDataHandleOpen( ARC_WER_PLTT,		heap_area );
	we_sys->hdl[ 4 ] = ArchiveDataHandleOpen( ARC_WER_CELL,		heap_area );
	we_sys->hdl[ 5 ] = ArchiveDataHandleOpen( ARC_WER_CELLANM,	heap_area );
...
BOOL WES_Delete(WE_SYS_PTR we_sys)
{
	...
	
	{
		...
		for (i = 0; i < WE_HDL_NUM; i++){
			ArchiveDataHandleClose( we_sys->hdl[ i ] );
		}
	}
...
//	GF_BGL_ScrClearCode( we_sys->bgl, BATTLE_FRAME_EFFECT, 64 );
	...
	ArcUtil_HDL_ScrnSet(we_sys->hdl[ WE_HDL_ARC_BATT_BG ],
					index_screen,
					we_sys->bgl,
...
static void WEST_POKEOAM_RES_LOAD(WE_SYS_PTR we_sys)
{
	...
	CATS_LoadResourcePlttWorkArcH(we_sys->pfd, FADE_MAIN_OBJ,
								 we_sys->btl_sys->csp, we_sys->poke_crp,
								 we_sys->hdl[ WE_HDL_ARC_BATT_OBJ ], PO_PLTT_ID, 0,
								 NNS_G2D_VRAM_TYPE_2DMAIN, 1, res_id[1]);

	CATS_LoadResourceCellArcH(we_sys->btl_sys->csp, we_sys->poke_crp,
							 we_sys->hdl[ WE_HDL_ARC_BATT_OBJ ], PO_CELL_ID, 0, res_id[2]);

	CATS_LoadResourceCellAnmArcH(we_sys->btl_sys->csp, we_sys->poke_crp,
							 	we_sys->hdl[ WE_HDL_ARC_BATT_OBJ ],	PO_CELLANM_ID, 0, res_id[3]);
}
...
static void WEST_CATS_CHAR_RES_LOAD(WE_SYS_PTR we_sys)
{
	...
	CATS_LoadResourceCharArcH(we_sys->btl_sys->csp, we_sys->crp[res_no],
							 we_sys->hdl[ WE_HDL_ARC_WER_CHAR ], arc_id, TRUE, NNS_G2D_VRAM_TYPE_2DMAIN, arc_id + EFF_ARC_ID_BASE);
}
...
static void WEST_CATS_PLTT_RES_LOAD(WE_SYS_PTR we_sys)
{
	...
	CATS_LoadResourcePlttWorkArcH(we_sys->pfd, FADE_MAIN_OBJ,
								 we_sys->btl_sys->csp, we_sys->crp[res_no],
							 	 we_sys->hdl[ WE_HDL_ARC_WER_PLTT ], arc_id, 0, NNS_G2D_VRAM_TYPE_2DMAIN, pal_num, arc_id + EFF_ARC_ID_BASE);	
}
...
static void WEST_CATS_CELL_RES_LOAD(WE_SYS_PTR we_sys)
{
	...
	CATS_LoadResourceCellArcH(we_sys->btl_sys->csp, we_sys->crp[res_no],
							 we_sys->hdl[ WE_HDL_ARC_WER_CELL ], arc_id, TRUE, arc_id + EFF_ARC_ID_BASE);	
}
...
static void WEST_CATS_CELL_ANM_RES_LOAD(WE_SYS_PTR we_sys)
{
	...
	CATS_LoadResourceCellAnmArcH(we_sys->btl_sys->csp, we_sys->crp[res_no],
							 we_sys->hdl[ WE_HDL_ARC_WER_CELLANM ], arc_id, TRUE, arc_id + EFF_ARC_ID_BASE);		
}
...
static void WEST_VOICE_PLAY(WE_SYS_PTR we_sys)
{
	...
	form_no = we_sys->btl_sys->form_no[ we_sys->btl_sys->at_client_no ];
//	OS_Printf("mons_no = %d we_sys\n", mons_no);

	PokeVoicePlaySet(we_sys->btl_sys->perap_voice, type, mons_no, form_no, pan, vol, we_sys->btl_sys->henshin_flag, we_sys->heap_area);
...
OAM_DROP_SYS* OAM_Drop_Init(BATTLE_WORK* bw, int heap, int client_no)
{
	...
	
	{
		...
		ARCHANDLE* hdl;
		hdl  = ArchiveDataHandleOpen( ARC_BATT_OBJ,  heap ); 
		...
		//	OS_Printf("pal id = %6d\n", res_id[ 1 ]);
			...
			CATS_LoadResourceCharArcH(wk->sys.csp, wk->sys.crp,
									 hdl, PO_CHAR_ID, 0,
									 NNS_G2D_VRAM_TYPE_2DMAIN, res_id[0]);

			CATS_LoadResourcePlttWorkArcH(wk->sys.pfd, FADE_MAIN_OBJ,
										 wk->sys.csp, wk->sys.crp,
										 hdl, PO_PLTT_ID, 0,
										 NNS_G2D_VRAM_TYPE_2DMAIN, 1, res_id[1]);

			CATS_LoadResourceCellArcH(wk->sys.csp, wk->sys.crp,
									 hdl, PO_CELL_ID, 0, res_id[2]);

			CATS_LoadResourceCellAnmArcH(wk->sys.csp, wk->sys.crp,
									 	hdl, PO_CELLANM_ID, 0, res_id[3]);
		}

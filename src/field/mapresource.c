...
void SetupMapResource(MAP_RESOURCE_PTR outMapResource)
{
	...
	model_handle = ArchiveDataHandleOpen( ARC_3D_STRUCT, HEAPID_FIELD );
	{
		...
		//ポインタ「now」は既に1行読み飛ばされた
		//modelTableの内容を指しているので次からStrTokはパス名取得になる
		for(i=0;i<work->Num;i++){
			//データはメモリの1番目から（0番目は個数）
			...
			{	//アニメセット
				...
				if (no < anime_num){
					model = NNS_G3dGetMdlByIdx(
						NNS_G3dGetMdlSet(outMapResource->model[no]), 0);
	...
	if( outMapResource->model[0] == NULL ){
		...
		/*
		 outMapResource->model[0] =
		 	ArchiveDataLoadMalloc( ARC_3D_STRUCT, 0, HEAPID_FIELD );
		*/

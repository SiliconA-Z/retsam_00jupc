...
typedef enum {
	...
	// タッチスクリーンの操作説明は表示の縦位置を自動調整するように変更
	GUIDETYPE_TOUCH,
	// ----------------------------------------------------------------------------
}INTRO_GUIDETYPE;
...
#define INTRO_MONSNO ( MONSNO_MIMITTO )	// PLATINUMでミミロルに変更
...
const BMPWIN_DAT opera_windata = {
...
const BMPWIN_DAT guide_windata = {
...
static BOOL Intro_BmpList( INTRO_DEMO_WORK * wk, INTRO_LISTTYPE type, int button_mode )
{
	...
	case LISTSET:
		...
		case LISTTYPE_NAME:
			...
			if( CasetteVersion == VERSION_DIAMOND || PM_VERSION == VERSION_PLATINUM ){
				list  = name_list_d;
	...
	case GUIDESET:
		...
		if( type == GUIDETYPE_GUIDE ){
			...
			if ( type == GUIDETYPE_TOUCH ) {
                // MatchComment: FontProc_GetPrintLineNum -> STRBUF_GetLines
				u32 linenum = STRBUF_GetLines(wk->msgstr);
				windata.pos_y = pos_y + siz_y / 2 - linenum;
				windata.siz_y = linenum * 2;
			} else {
				windata.pos_y = pos_y;
				windata.siz_y = siz_y;
...
static void Intro_GraphicLoad( INTRO_DEMO_WORK* wk )
{
	...
	{//パレット（バージョンによって変化）
		...
		if( CasetteVersion == VERSION_PLATINUM){
			palIDm = NARC_intro_back_m_pl_NCLR;
			palIDs = NARC_intro_back_s_pl_NCLR;
		} else if( CasetteVersion == VERSION_DIAMOND ){
...
//	PLATINUMで表示位置をもう少し上に変更
	...
	GF_BGL_ScrFill( wk->bgl, frame_s, 0, 0, 0, 32, 24, pal_s); 
	GF_BGL_ScrWrite( wk->bgl, frame_s, monsscr, 11, 7, 10, 10 ); 
	Intro_BGscrPalChange( wk, frame_s, pal_s );
...
static void Intro_GraphicPalAnmPOKE( INTRO_DEMO_WORK* wk )
{
	if( wk->work[8] > 0 ){
		wk->work[8]--;
		// 反映カラーを作成
		...
		GF_BGL_PaletteSet( INTRO_PPL2_M_FRAME, wk->p_poke_colbuf_wk, PALSIZE, PALSIZE*INTRO_PPL2PLT_M_NUM );
		GF_BGL_PaletteSet( INTRO_PPL1_S_FRAME, wk->p_poke_colbuf_wk, PALSIZE, PALSIZE*INTRO_PPL1PLT_S_NUM );
	}
}
	...
	case 4:
		{
			...
			int plus = wk->work[6] * GRABITY_VAL * wk->work[7];
			int minus = GRABITY_VAL * wk->work[7] * wk->work[7] / 3;
			int y = plus - minus;
			...
			if(( wk->work[5] > 0 ) && ( y <= 0 )){
				...
				*seq = 5;
				...
				wk->work[7]++;
				GF_BGL_ScrollSet( wk->bgl, INTRO_PPL2_M_FRAME, GF_BGL_SCROLL_X_DEC, 2 );
				GF_BGL_ScrollSet( wk->bgl, INTRO_PPL2_M_FRAME, GF_BGL_SCROLL_Y_SET, y );
				wk->work[5] = y;
			}
		}
	...
	case 5:
		{
			...
			if(( wk->work[5] > 0 ) && ( y <= 0 )){
				...
				*seq = 6;
				...
				wk->work[7]++;
				GF_BGL_ScrollSet( wk->bgl, INTRO_PPL2_M_FRAME, GF_BGL_SCROLL_X_INC, 4 );
				GF_BGL_ScrollSet( wk->bgl, INTRO_PPL2_M_FRAME, GF_BGL_SCROLL_Y_SET, y );
				wk->work[5] = y;
			}
		}
